/**
 * PancakeSwap V3 fee wrapper quotes (BSC, ERC20→ERC20). Read-only; no signing.
 */
...
/**
 * Single-tier quote via SwaperexPancakeV3FeeWrapper (net output after protocol fee).
 */
export async function getPancakeWrapperQuote(
  tokenIn: string,
  tokenOut: string,
  amountIn: string,
  ...
  if (!cfg.enabled || !cfg.wrapperAddress) {
    throw new Error('Pancake fee wrapper is not enabled or not configured');
  }
  ...
  const wrapper = new Contract(cfg.wrapperAddress, WRAPPER_QUOTE_ABI, rpc);
  ...
  try {
    ...
    console.warn('[PancakeWrapperQuote] staticCall failed, retrying once:', firstErr);
...
/** Best wrapper quote across the same fee tiers as direct Pancake best-price (500 / 2500 / 10000). */
export async function getBestPancakeWrapperQuote(
  tokenIn: string,
  tokenOut: string,
  amountIn: string,
  ...
  const results = await Promise.allSettled(
    feeTiers.map((fee) => getPancakeWrapperQuote(tokenIn, tokenOut, amountIn, fee)),
  );
