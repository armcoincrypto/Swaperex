...
class DASHHDWallet(HDWalletProvider):
    """DASH HD Wallet using BIP44.

    Derivation path: m/44'/5'/0'/change/index
    Address format: P2PKH (X... on mainnet)

    DASH uses standard BIP44 with coin type 5.
    """
    ...
    def _validate_xpub(self) -> None:
        """Validate DASH xpub and initialize BIP32 context."""
        if not self._xpub:
            return

        valid_prefixes = ["xpub", "tpub", "drkp", "drkv"]
        if not any(self._xpub.startswith(p) for p in valid_prefixes):
            raise ValueError("Invalid DASH xpub prefix")
        ...
        if HAS_BIP_UTILS:
            try:
                ...
                raise ValueError(f"Invalid DASH xpub: {e}")
    ...
    @property
    def coin_type(self) -> int:
        return 5  # SLIP-44 for DASH

    @property
    def purpose(self) -> int:
        return 44  # BIP44 for DASH (not segwit)
    ...
    def _derive_with_bip32(self, index: int, change: int) -> AddressInfo:
        ...
        # DASH uses P2PKH addresses
        # Mainnet: 0x4C (76) -> addresses start with 'X'
        # Testnet: 0x8C (140) -> addresses start with 'y'
        net_ver = b'\x8c' if self._testnet else b'\x4c'
    ...
    def _derive_simulated(self, index: int, change: int) -> AddressInfo:
        ...
        # DASH mainnet addresses start with 'X', testnet with 'y'
        prefix = "y" if self._testnet else "X"
