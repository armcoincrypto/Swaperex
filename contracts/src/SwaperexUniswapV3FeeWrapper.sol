...
/// @title SwaperexUniswapV3FeeWrapper
/// @notice v1: Ethereum Uniswap V3 `SwapRouter02` only, ERC20→ERC20, output-side fee. Immutable config.
contract SwaperexUniswapV3FeeWrapper is ReentrancyGuard {
    using SafeERC20 for IERC20;

    /// @notice Uniswap V3 SwapRouter02 (immutable).
    address public immutable ROUTER;
    /// @notice Uniswap V3 QuoterV2 (immutable).
    ...
    /// @notice Some UIs use this sentinel for “native”; v1 does not support native ETH.
    ...
    ///      try/catch (see Uniswap `QuoterV2.sol`). Use top-level `eth_call` off-chain; do not `STATICCALL` this.
