...
contract SwaperexUniswapV3FeeWrapperV2ForkTest is Test {
    ...
    function _pickWethUsdtPoolFee() internal returns (uint24) {
        ...
        for (uint256 i = 0; i < fees.length; i++) {
            IUniswapV3QuoterV2.QuoteExactInputSingleParams memory p = IUniswapV3QuoterV2.QuoteExactInputSingleParams({
                tokenIn: WETH, tokenOut: USDT, amountIn: 1 ether, fee: fees[i], sqrtPriceLimitX96: 0
            });
        ...
        revert("fork: no WETH/USDT Uniswap v3 pool found");
    }
    ...
    function testFork_ethToUsdt_userNet_treasuryFee() public forkOnly {
        ...
        (uint256 qGross,, uint256 qNet,,,) = w.quoteExactInputSingleERC20(WETH, USDT, poolFee, amountIn, 0);
        ...
        (uint256 gExec, uint256 feeExec, uint256 netExec) = w.swapExactInputSingleEthForTokens{value: amountIn}(
            USDT, poolFee, amountIn, minNet, block.timestamp + 3600, 0
        );
    ...
    /// @dev B) USDT → ETH: user receives net ETH; treasury receives fee in ETH.
    function testFork_usdtToEth_userEth_treasuryFee() public forkOnly {
        deal(USDT, user, 10_000e6);
        uint256 amountIn = 2_000e6;

        (uint256 qGross,, uint256 qNet,,,) = w.quoteExactInputSingleERC20(USDT, WETH, poolFee, amountIn, 0);
        uint256 minNet = (qNet * 99) / 100;
        ...
        vm.startPrank(user);
        IERC20(USDT).approve(address(w), type(uint256).max);
        (uint256 gExec, uint256 feeExec, uint256 netExec) =
            w.swapExactInputSingleTokensForEth(USDT, poolFee, amountIn, minNet, block.timestamp + 3600, 0);
        vm.stopPrank();
    ...
    function testFork_ethToUsdt_smallAmount_noRevert() public forkOnly {
        ...
        (,, uint256 qNet,,,) = w.quoteExactInputSingleERC20(WETH, USDT, poolFee, amountIn, 0);
        uint256 minNet = (qNet * 95) / 100;

        vm.prank(user);
        w.swapExactInputSingleEthForTokens{value: amountIn}(USDT, poolFee, amountIn, minNet, block.timestamp + 3600, 0);
    }
    ...
    function testFork_ethToUsdt_slippage_reverts() public forkOnly {
        ...
        vm.prank(user);
        vm.expectRevert(SwaperexUniswapV3FeeWrapperV2.SlippageExceeded.selector);
        w.swapExactInputSingleEthForTokens{value: amountIn}(USDT, poolFee, amountIn, impossibleMin, block.timestamp + 3600, 0);
    }
}
