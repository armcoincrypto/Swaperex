...
/// @notice Broadcasts `SwaperexUniswapV3FeeWrapperV2` on Ethereum (or the forked chain selected by RPC).
/// @dev Required env:
///      - `UNISWAP_V3_SWAP_ROUTER02` — SwapRouter02
///      - `UNISWAP_V3_QUOTER_V2` — QuoterV2
///      - `WETH` — canonical WETH on the target chain
///      - `TREASURY` — receives protocol fees (ERC20 or ETH)
...
contract DeployUniswapWrapperV2 is Script {
    function run() external returns (SwaperexUniswapV3FeeWrapperV2 wrapper) {
        ...
        vm.startBroadcast();
        wrapper = new SwaperexUniswapV3FeeWrapperV2(initialOwner, router, quoter, weth, treasury, feeBps);
        vm.stopBroadcast();

        console2.log("SwaperexUniswapV3FeeWrapperV2:", address(wrapper));
