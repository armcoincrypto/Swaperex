#!/usr/bin/env node
/**
 * P16.6 — Mobile WalletConnect connectivity certification assist.
 * Connectivity only — no swaps, approvals, or broadcasts.
 *
 * Checklist exercised (automated where possible):
 * - WalletConnect QR modal opens
 * - Deep-link entry point present
 * - Reconnect / disconnect UI reachable
 * - Session storage hint detection
 *
 * Human handset validation (MetaMask Mobile, Trust Wallet) documented in audit output.
 *
 * Usage:
...
async function main() {
  ...
  if (opts.skipBrowser) {
    ...
    report.notes.push('Browser checks skipped (P16_SKIP_BROWSER=1).');
  } else {
    const playwrightHref = pathToFileURL(
      path.join(REPO_ROOT, 'frontend/node_modules/playwright/index.mjs'),
    ).href;
    ...
    try {
      for (const vp of VIEWPORTS) {
        ...
        report.walletChecks.connectWalletCtaVisible = await walletBtn.isVisible().catch(() => false);
